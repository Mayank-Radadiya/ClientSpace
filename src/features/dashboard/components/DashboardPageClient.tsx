...
function eventIcon(eventType: string) {
  if (eventType.includes("invoice")) return { Icon: ReceiptText, color: "text-violet-500", bg: "bg-violet-500/10" };
  if (eventType.includes("project")) return { Icon: FolderOpen, color: "text-blue-500", bg: "bg-blue-500/10" };
  if (eventType.includes("task") && eventType.includes("complete")) return { Icon: CheckCircle2, color: "text-emerald-500", bg: "bg-emerald-500/10" };
  if (eventType.includes("task")) return { Icon: Clock, color: "text-amber-500", bg: "bg-amber-500/10" };
  if (eventType.includes("file")) return { Icon: FileText, color: "text-cyan-500", bg: "bg-cyan-500/10" };
  if (eventType.includes("client")) return { Icon: Users, color: "text-rose-500", bg: "bg-rose-500/10" };
...
function ChartSkeleton({ height = "h-64" }: { height?: string }) {
  return (
    <div className={cn("animate-pulse rounded-2xl border border-neutral-100 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900", height)}>
...
function KpiCard({ label, value, sub, icon: Icon, gradient, trend, delay = 0 }: KpiCardProps) {
  return (
    <motion.div
      variants={item}
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    in_progress: "bg-violet-100 text-violet-700 dark:bg-violet-950/50 dark:text-violet-300",
  ...
  return (
    <span className={cn("rounded-full px-2.5 py-0.5 text-xs font-medium", styles[status] ?? styles.not_started)}>
...
export function DashboardPageClient({ userName }: DashboardPageClientProps) {
  ...
  const chartData = (revenueData ?? []).map((r: { month: string; amountCents: number }) => ({
    month: r.month,
  ...
  return (
    <PageLayout bleed>
        ...
        >
          <div>
            <h1 className="font-brand text-2xl font-bold tracking-tight text-neutral-900 dark:text-white md:text-3xl">
        ...
        >
          ...
          <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-2">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-semibold text-neutral-900 dark:text-white">
                Recent Activity
              </h2>
          ...
          <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-3">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-semibold text-neutral-900 dark:text-white">
                Recent Projects
              </h2>
                      ...
                      >
                        <td className="py-3 pr-4">
                          <div className="flex items-center gap-2">
                            <div className="h-2 w-2 shrink-0 rounded-full" style={{ background: STATUS_COLORS[proj.status] ?? "#6b7280" }} />
